...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const storedCredentials = localStorage.getItem('lms_credentials');
    if (storedUser) {
      try {
    ...
    if (storedCredentials) {
      try {
        ...
        localStorage.removeItem('lms_credentials');
      }
    }
    setLoading(false);
  }, []);
  ...
  useEffect(() => {
    ...
    if (Object.keys(userCredentials).length === 0) {
      ...
      localStorage.setItem('lms_credentials', JSON.stringify(defaultCredentials));
    }
  }, [userCredentials]);
  ...
  const updateProfile = async (updates: { username?: string; password?: string }): Promise<boolean> => {
    ...
    try {
      ...
      if (updates.password) {
        ...
        localStorage.setItem('lms_credentials', JSON.stringify(updatedCredentials));
      }
